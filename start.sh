...
#exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" -Dlog4j.configurationFile="${APP_DIR}/log4j2.xml" "${APP_DIR}/document-generator-consumer.jar"

# debug settings
exec java -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=21095 -jar -Dserver.port="${PORT}" "${APP_DIR}/document-generator-consumer.jar"