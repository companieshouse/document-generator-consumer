...
locals {
  stack_name                 = "document-generation" # this must match the stack name (cluster) the service deploys into
  name_prefix                = "${local.stack_name}-${var.environment}"
  global_prefix              = "global-${var.environment}"
  service_name               = "document-generator-consumer"
  service_name_comparison     = "document-generator-consumer-comparison"
  container_port             = "8080"
  docker_repo                = "document-generator-consumer"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  healthcheck_path           = "/document-generator-consumer/healthcheck" # healthcheck path for document-generator-consumer
  healthcheck_matcher        = "200"
  vpc_name                   = local.stack_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "document-generator-consumer.env"
  app_environment_filename_comparison    = "document-generator-consumer-comparison.env"
